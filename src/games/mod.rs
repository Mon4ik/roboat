...
const PLACE_DETAILS_API: &str = "https://games.roblox.com/v1/games/multiget-place-details?placeIds={place_ids}";
...
impl Client {
    ...
    /// # }
    /// ```
    pub async fn place_details_bulk(&self, place_ids: Vec<u64>) -> Result<Vec<PlaceDetails>, RoboatError> {
        ...
        let formatted_url = PLACE_DETAILS_API
            .replace("{place_ids}", &formatted_place_ids);
    ...
    pub async fn place_details(&self, universe_id: u64) -> Result<PlaceDetails, RoboatError> {
        ...
        let place_details = places_details
            .first()
            .ok_or(RoboatError::BadRequest)?;
